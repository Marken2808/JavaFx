...
public class NameResource {
    ...
    public ResponseEntity<Name> updateName(@PathVariable(value = "id", required = false) final Long id, @Valid @RequestBody Name name)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, name.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Name> partialUpdateName(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, name.getId().toString())
        );
    }
     ...
     */
    @GetMapping("/names")
    public List<Name> getAllNames(@RequestParam(required = false) String filter) {
        if ("customer-is-null".equals(filter)) {
            log.debug("REST request to get all Names where customer is null");
            return StreamSupport
                .stream(nameRepository.findAll().spliterator(), false)
                .filter(name -> name.getCustomer() == null)
